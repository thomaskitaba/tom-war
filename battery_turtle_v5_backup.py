...
def spawn_ufo(ufo_x_coord, ufo_y_coord):
	
	ufo_image_l = [pygame.transform.scale(ufo_image, (70,70))]
	screen.blit(ufo_image_l[0], (ufo_x_coord, ufo_y_coord))
...
def display_high_score(h_score_w, h_score_h):
	...
	#screen.blit(high_score_bg_l[0], (100,100))
		...
		screen.blit(lbl_10, (h_score_w - 490, 540))
...
#============= JET LEVEL =============	
			...
			# movment 
				...
				# ==== FIND ROTATION ===
				#	sin_jet _teta = hypotenus/opposit
				#hypotenus = int(math.sqrt((rand_jet_x - batteryx)**2 + (rand_jet_y - batteryy)**2))
				
				#opposite = int(math.sqrt((rand_jet_x - batteryy)**2 + (batteryy -batteryy)**2))
				#if hypotenus == 0:
					#hypotenus += 1
					
				#opp_hyp = int(opposite / hypotenus)
				#sinteta = math.asin(opp_hyp)
				#rotation_angle = math.degrees(sinteta)
