...
def fire_missiel(left_missiel_x, right_missiel_x,missiel_y):
	#if fired_missiel== True and weapon[0] =="missiel":
		#----- moves the missiel with the tank---
	#if weapon_upgraded and not fired_missiel:
	if weapon_l[0] == "missiel":
		if weapon_upgraded_l[0] == 1 and not fired_missiel:
			screen.blit(missiel_img_l[0],( left_missiel_x - 30,missiel_y))
			screen.blit(missiel_img_l[1],(right_missiel_x + 20,missiel_y))
		#-----------
		#----- launchs the missiel---
		if weapon_upgraded_l[0] == 1 and fired_missiel:
			screen.blit(missiel_img_l[0],( left_missiel_x, missiel_y))
			screen.blit(missiel_img_l[1],(right_missiel_x,missiel_y))
	
	
def fire_missiel_(left_missiel_x, right_missiel_x,missiel_y):
	#if fired_missiel== True and weapon[0] =="missiel":
		#----- moves the missiel with the tank---
	#if weapon_upgraded and not fired_missiel:
	if weapon_l[0] == "missiel":
...
#====== WEAPON   UPGRADE ============					
			...
			if score == 2:
			 	spawn_upgrader_l[0] = 1
			 	weapon_upgrader_visible = True
			 	
			if spawn_upgrader_l[0] == 1 and weapon_upgrader_visible == True:
				weapon_upgrader_y += 10
				weapon_upgrader(weapon_upgrader_x,weapon_upgrader_y)
			if weapon_upgrader_y >= 600:
					weapon_upgrader_y = 0
					weapon_upgrader_y += 100
					weapon_upgrader_y = -100
					
					spawn_upgrader_l[0] == 0
					weapon_upgrader_visible = False
	...
	#====== LIFE LOST COLLISION======
		...
		#missiel()
		if weapon_upgraded_l[0] == 1 and not fired_missiel:
			left_missiel_x =  missiel_x- 10
			right_missiel_x =  missiel_x 
		...
		if not game_over and weapon_upgraded_l[0] == 1:
			fire_missiel(left_missiel_x, right_missiel_x,missiel_y)
