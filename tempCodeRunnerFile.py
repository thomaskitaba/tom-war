...
def bush_tree():
	...
	if level_4:
		...
		screen.blit(bush_l [0], (bush_2_x, bush_2_y))
		screen.blit(tree_l[0], (tree_2_x, tree_2_y))
...
enemy_tank_hit = False

enemy_tank_bullet_d = [0]
enemy_tank_left_missiel_d = [0]
enemy_tank_right_missiel_d = [0]
...
life_bar_tank_start_x_l = [enemy_tank_x ]
life_bar_tank_start_y_l = [enemy_tank_y - 45]
life_bar_tank_end_x_l= [enemy_tank_x + 100]
life_bar_tank_end_y_l = [enemy_tank_y - 45]
life_bar_red_tank_end_x_l= [enemy_tank_x + 100]


def life_bar():
	#if enemy_tank_hit :
	pygame.draw.line(screen, (255,0,0),  (life_bar_tank_start_x_l[0],life_bar_tank_start_y_l[0]), (life_bar_red_tank_end_x_l[0],life_bar_tank_end_y_l[0]), 10)

		#draw the red bar
	pygame.draw.line(screen, (0,255,0),  (life_bar_tank_start_x_l[0],life_bar_tank_start_y_l[0]), (life_bar_tank_end_x_l[0],life_bar_tank_end_y_l[0]), 10)
...
def does_file_exist(file_path):
	# Check if the file exists (implementation not shown)
	pass

def record_high_score(scorer_name, end_score):
	col_name = ["name", "score"]
	default_rows = [['a', 10], ['b', 9], ['c', 8], ['d', 7], ['e', 6], ['f', 5], ['g', 4], ['h', 3], ['i', 2], ['j', 1]]

	if does_file_exist("High Score.csv"):
		with open("High Score.csv", 'r') as game_high_score:
		...
		with open("High Score.csv", 'w') as High_Score_file:
...
def display_high_score(h_score_w, h_score_h):
	...
	if game_over:
		...
		pygame.draw.rect(screen,(255,19,0), pygame.Rect(70,90, 560, 540), 4)
	...
	#screen.blit(high_score_bg_l[0], (100,100))
		...
		if does_file_exist:
			with open("High Score.csv") as hs_file:
...
def display_screen_2():
	...
	enter_name = my_medium_font.render(" PLAYER NAME ", 1,(0,255,0))
	screen.blit(enter_name,(145, 50))
...
while typing:
	...
	for events in pygame.event.get():
		...
		if events.type == pygame.KEYDOWN :
			...
			player_name = my_small_font.render(player, 1,(255,0, 0))
			screen.blit(player_name, (110, 160))
		pygame.display.update()
...
def player():
	...
	screen.blit(player_name, (470, 30))
#==================================
...
mini_big_font = pygame.font.SysFont("Comic Sans MS", 60)
# left title
...
def show_statistics():
	...
	if score > 0:
		...
		shoot_statstics_l[0]  = round(round((turtle_shoot_l[0] /total_enemies_l[0] ), 2) ,2)*100 #turtle_shoot_l[0]
	...
	total_enemies_r = game_data_small_font.render( total_enemies , 1, (0,0,255))

	shoot_statstics_r = game_data_small_font.render( shoot_statstics , 1, (0,0,255))

	total_enemies_distroyed_r = game_data_small_font.render( total_enemies_distroyed , 1, (0,0,255))

	total_enemies_missed_r = game_data_small_font.render( total_enemies_missed , 1, (0,0,255))


	screen.blit(shoot_statstics_r, (420,15))
	screen.blit(total_enemies_r, (420, 50))

	screen.blit(total_enemies_distroyed_r, (350,50))
	screen.blit(total_enemies_missed_r, (350,70))
...
def show_game_data():
	...
	letter_T_r = mini_big_font.render("T", 2, (0,0,0))

	screen.blit(mini_turtle_l[0], (130,0))
	screen.blit(mini_missile_l[0], (180,0))
	screen.blit(mini_jet_l[0], (235,0))
	screen.blit(mini_ufo_l[0], (300,0))
	screen.blit(letter_T_r, (350,0))

	screen.blit(lable_accuracy, (380, 0))
	screen.blit(lable_total_enemies, (380, 30))
	...
	screen.blit(jet_shoot_r, (250,50))
	screen.blit(ufo_shoot_r, (300,50))

	### MISSED - MISSID -MISSED
	screen.blit(turtle_missed_r, (150,70))
	screen.blit(tank_missile_missed_r, (200,70))
	screen.blit(jet_missed_r, (250,70))
	screen.blit(ufo_missed_r, (300,70))
...
if  game_over == False:
	while running:
		# Initialize timer
		# set level option
		level_option_main = False
		level_option_test = True
		...
		for event in pygame.event.get():
			...
			if event.type == pygame. MOUSEBUTTONDOWN:
				...
				if game_over == True:
					...
					#reset enemy_tank hit bar to default
					life_bar_tank_end_x_l[0] = enemy_tank_x + 100
...
# after enemy tank reachs its full size fire missiel
...
# -------- ENEMY TANK HIT BY MISSIEL ------
		# distance between enemy tank and bullet
		enemy_tank_bullet_d[0]= int(math.sqrt((enemy_tank_x - bulletx)**2 + (enemy_tank_y - bullety)**2))

		enemy_tank_left_missiel_d[0] = int(math.sqrt((enemy_tank_x - left_missiel_x)**2 + (enemy_tank_y - missiel_y)**2))
		#hit by right missiel
		enemy_tank_right_missiel_d[0] = int(math.sqrt((enemy_tank_x-right_missiel_x)**2 + (enemy_tank_y - missiel_y)**2))

		if ((enemy_tank_bullet_d[0] >0 and enemy_tank_bullet_d[0] <= 50)or (enemy_tank_left_missiel_d[0] > 0 and enemy_tank_left_missiel_d[0] <= 50) or ( enemy_tank_right_missiel_d[0] > 0 and enemy_tank_right_missiel_d[0] <= 100 )) and enemy_tank_level:

			life_bar_tank_end_x_l[0] -= 1
			score +=1

			enemy_tank_hit = False

		if life_bar_tank_end_x_l[0] - life_bar_tank_start_x_l[0] <= 0:
			# explode tank EFFECT
			# big Expl]sion EFFECT
			life_bar_tank_end_x_l[0]
			score += 10
	...
	#	if enemy_tank_hit and enemy_tank_level:
		#	score +=10
			#enemy_tank_hit == False


		#if bullety < 10 or missiel_y < 10:
			#	enemy_tank_hit = False
