...
# ----- ENABLE and  DISABLE BULLETS HITING CAPABILITY
		...
		if (rand_jet_y  > 0 and rand_jet_y  <=500) or (rand_jet_x - batteryx >=10):
			hit_jet = True
		else:
			hit_jet = False
...
#	============ JET HIT BY MISSIEL =======
		if ((missiel_1_on_jet > 0 and missiel_1_on_jet < 50) or (missiel_2_on_jet > 0 and missiel_2_on_jet < 50) or (bullet_on_jet > 0 and bullet_on_jet < 50) ) and hit_jet== True:
			score += 1
			#saved_jet_coord.clear()
			
			if level_3 or level_4: # to remove the blast on the left corner of the screen
				car_explode_sound_0.play()
				jet_explosion(rand_jet_x, rand_jet_y)
			rand_jet_x = 0
			rand_jet_y =  int(random.randint(10,200))

#======= TURTLE HIT BY BULLET ========
...
#====== JET /TANK _ MISSIEL COLLISION======
		
					#if hit_tank_missiel
				missiel_tank_d = int(math.sqrt((tank_missiel_image_l[0] - batteryx)**2 + (tank_missiel_image_l[1]-batteryy)**2))
		
		# when hit by jet
		jet_tank_d = int(math.sqrt((batteryx -rand_jet_x)**2 + (batteryy - rand_jet_y)**2))
		# HIT BY JET JET JEYLT
		if jet_tank_d <= 50:
		...
		if missiel_tank_d > 0 and missiel_tank_d <=200:
...
#--------------------------------------------------		
		...
		player()
		#get_coordinate(saved_battery_coord_l[0],saved_battery_coord_l[1])
