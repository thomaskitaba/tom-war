...
def display_high_score(h_score_w, h_score_h):
	...
	if game_over:
		...
		pygame.draw.rect(screen,(255,19,0), pygame.Rect(70,80, 560, 540))
...
if  game_over == False:
	while running:
		...
		#if score >= 60: # to be deleyed at the end
		if score >= 11:
...
# ----- ENABLE and  DISABLE BULLETS HITING CAPABILITY
		...
		# JET hiting capablity
		jet_bullet_distance = int(math.sqrt((rand_jet_x -bulletx)**2 + (rand_jet_y - bullety)**2))
		if jet_bullet_distance > 20:
			hit_jet = True
		else:
			hit_jet = False
# ------------------------------------------
# JET HIT BY MISSIEL OR BULLET
		if ((missiel_1_on_jet > 0 and missiel_1_on_jet < 50) or (missiel_2_on_jet > 0 and missiel_2_on_jet < 50) or (bullet_on_jet > 0 and bullet_on_jet < 50) ) and hit_jet== True:
			score += 1
			
			#------- for explosion -------
			jet_hit_time = pygame.time.get_ticks()
			# save coordinate for explosion 
			saved_ufo_coord_l.clear()
			
			saved_jet_coord_l.append(rand_jet_x)
			saved_jet_coord_l.append(rand_jet_y)
			
			
			if current_time - jet_hit_time < 300:
				jet_explosion(saved_jet_coord_l[0], saved_jet_coord_l[1])
				saved_jet_coord_l.clear()
			# respawn tank missiel to its orignal position
			rand_jet_x = 0
			rand_jet_y =  int(random.randint(10, 200))
...
# TANK MISSIEL HIT BY MISSIEL OR BULLET
		if ((missiel_1_on_tank_missiel > 0 and missiel_1_on_tank_missiel < 50) or (missiel_2_on_tank_missiel > 0 and missiel_2_on_tank_missiel < 50) or (bullet_on_tank_missiel > 0 and bullet_on_tank_missiel < 50) ) and hit_tank_missiel == True:
			...
			saved_ufo_coord_l.append(tank_missiel_image_l[1])
			
			if current_time - tank_missiel_hit_time < 300:
				tank_missiel_explosion(saved_ufo_coord_l[0], saved_ufo_coord_l[1])
		...
		if life == 0:
			...
			if not game_over and fired == False:
...
#--------------------------------------------------		
		...
		display_result(saved_battery_coord_l[0],saved_battery_coord_l[1])
